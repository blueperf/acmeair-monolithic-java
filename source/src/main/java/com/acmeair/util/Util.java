...
public class Util {
      ...
      while (true) {
        try {
          ...
          try {
            ...
            System.out.println("HEARTBEAT FAILED AT " + e.getClass() + " WITH ERROR : "
                + e.getMessage() + " RE-REGISTERING");
          }
        } catch (Exception e) {
          int sleepTime = 10000;
          System.out.println("REGISTRATION FAILED AT " + e.getClass() + " WITH ERROR : "
              + e.getMessage() + " RE-REGISTERING AFTER " + sleepTime / 1000 + " sec  SLEEP");
          try {
  ...
  public static void sendHeartbeat(String heartbeatUrl, String BEARER_TOKEN, int SLEEP_TIME)
      throws Exception {
    URL url;
    while (true) {
  ...
  public static String getServiceProxy() {
    ...
    while (true) {
      try {
        ...
        System.out.println("SP Response Code : " + conn.getResponseCode() + " SP Response : "
            + response.toString());

        JSONParser parser = new JSONParser();
        JSONObject responseJson = (JSONObject) parser.parse(response.toString());
        JSONArray instancesJson =
            (JSONArray) parser.parse(responseJson.get("instances").toString());
        JSONObject instanceJson = (JSONObject) parser.parse(instancesJson.get(0).toString());
        JSONObject endpointJson =
            (JSONObject) parser.parse(instanceJson.get("endpoint").toString());
